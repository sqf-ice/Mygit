...
typedef enum {
    ...
    SYS_EVENT_TAMPER,      /*安全触发*/
    SYS_EVENT_REBOOT,      /*通知系统进行重启自检*/
    SYS_EVENT_MAX,
} EM_SYS_EVENT;
...
/**
 *@brief	获取辅CPU通知事件，每次返回一个事件，可循环调用直至取到"SYS_EVENT_NONE"事件为止
 *@retval   event 输出当前的通知事件，取值见"EM_SYS_EVENT"
 *			len 返回的事件消息长度，每个事件输出的通知信息最大不超过"MAX_SYS_EVENT_DATA_LEN"(128)字节
 *			out_data 返回的事件数据
 *@return
 *@li	NDK_OK			操作成功
 *@li	\ref NDK_ERR_PARA "NDK_ERR_PARA" 		参数错误(参数为NULL)
*/
 ...
 */
  ...
  *@return
  *@li   NDK_OK                  操作成功返回
  *@li   \ref NDK_ERR_PARA           参数非法(psBuf、punNum为NULL)
  *@li   \ref NDK_ERR                操作失败
 */
 int NDK_FsDir(const char *  psPath,  char *  psBuf, uint *  punNum);
