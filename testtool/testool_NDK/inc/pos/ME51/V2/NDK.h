...
*/
typedef enum {
	PRN_TTF_ATTRIBUTE_NORMAL = 1<<0,			/**<字体正常显示*/
	PRN_TTF_ATTRIBUTE_REVERSE = 1<<1,		/**<字体反向显示*/
...
/**
 *@brief    Set TTF print mode.
 *@param    emNewattr   New TTF print mode
 *@retval   pemOldattr  Old TTF print mode
 *@li   NDK_OK  Success
*/
int NDK_PrnSetTrueTypeFontAttr(EM_PRN_TTF_ATTRIBUTE  emNewattr, EM_PRN_TTF_ATTRIBUTE  *pemOldattr);

/**
 *@brief    Set print mix mode.
 *@param    emNewMixMode
                        0:      no mix mode
                        others:    mix mode
 *@retval
...
/**
 *@brief     从父进程读取子应用的参数
 *@details   这个接口只能和DK_AppStartChildProcess 配合使用
 *               父进程调用NDK_AppGetDatafromParentProcess
 *               外部子程序代码执行中调用NDK_AppGetDatafromParentProcess
 *               不支持多线程使用这个接口
 *@param    pData  返回给子应用的 数据
 *@param    pnLen  数据长度
 *@return
 *@li   NDK_OK              操作成功
 *@li   NDK_ERR_PARA        参数非法
 ...
 *@li	NDK_OK				操作成功
*@li	\ref NDK_ERR_PARA "NDK_ERR_PARA"	参数非法(pszFile为NULL,音频文件不存在)
*@li	\ref NDK_ERR "NDK_ERR"       		操作失败(加载音频文件失败 澹 )
*/
