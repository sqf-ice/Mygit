...
typedef	enum  {
    ...
    STATUSBAR_DISP_ALL = 1,  		/**<显示状态栏(时间、电量、无线信号)*/
	STATUSBAR_DISP_TIME = 2, 		/**<状态栏显示时间*/			
...
*/
	typedef enum {
		...
		SYS_HWINFO_GET_POS_TUSN,			/**<取 TUSN>*/
	} EM_SYS_HWINFO;
...
RTC自动唤醒MCU
*/
int NDK_SysSetWeakupDelay(uint second);
...
/**
 *@brief    蓝牙主设备请求连接
 *@param    pszMac： 请求连接的设备的mac地址，为NULL时，默认为请求连接与POS配对的最后一个设备(在低端上目前一般使用NULL)
 *@param    nDevType：连接方式 0:默认 1：SPP 2：BLE 低端蓝牙仅支持0或1，不支持BLE的回联
 *@param    nTimeoutMs：超时时间，单位为ms
 *@return
 *@li   NDK_OK              操作成功
 *@li   \ref NDK_ERR_NOT_SUPPORT   参数错误，nDevType不为0且不为1
 *@li   \ref NDK_ERR        操作失败，pszMac错误
 */
int NDK_BTMasterConnect(const char *pszMac, int nDevType, int nTimeoutMs);
...
int NDK_MFI_CHECK(void);
int NDK_BTMfiParamSet(const char *factory, const char *protocol, const char *bundleID);
int NDK_BTSetBleStatus(char status);
/**
 *@brief    蓝牙最大记录信息个数设置
 *@param    num:个数
 *@return
 *@li   NDK_OK              操作成功
 *@li   \ref NDK_ERR_PARA   num不在0~5之内
 *@li   \ref NDK_ERR_IOCTL  "NDK_ERR_IOCTL" 驱动接口调用错误
 */

int NDK_BTSetMaxBondedDevices(uint num);
#endif
/* End of this file */
