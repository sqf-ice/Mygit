...
//add by zengxiany for iso15693
#define FLAG_READ_SINGLE_BLOCK		0x22
#define FLAG_READ_Multiple_BLOCK	0x22
#define FLAG_WRITE_SINGLE_BLOCK		0x22
...
typedef struct
{
    uchar flags; 						/*!< flag byte of response */
    uchar dsfid; 						/*!< Data Storage Format Identifier */
    uchar uid[ISO15693_UID_LENGTH]; 	/*!< UID of the PICC */
    uint crc; 							/*!< CRC of response */
...
{
    uchar flags; 						/*!< flag byte of response */
    uchar infoFlags; 					/*!< info flags */
    uchar uid[ISO15693_UID_LENGTH]; 	/*!< UID of the PICC */
    uchar dsfid;  						/*!< Data Storage Format Identifier */
    uchar afi; 							/*!< Application Family Identifier */
    uchar memNumBlocks; 				/*!< number of blocks available */
    uchar memBlockSize; 				/*!< number of bytes per block */
    uchar icReference; 					/*!< IC reference field */
...
typedef struct
{
    uchar flags;
    uchar errorCode;
    uchar securityStatus; 							/*< security status byte */
    uchar blocknr; 
    uchar data[ISO15693_MAX_MEMORY_BLOCK_SIZE]; 	/*!< the content */
    uchar actualSize; 								/*!< actual size of \a data */
}iso15693PiccMemoryBlock_t;

typedef struct {
	iso15693ProximityCard_t* cards;
	iso15693PiccSystemInformation_t* sysInfo;
	iso15693PiccMemoryBlock_t* memBlock;		//change to ptr
    iso15693NumSlots_t slotcnt;
	uchar maskLength;
	uchar *mask;
	uchar maxCards;
    uchar cardsFound;	

	uchar flags;			//read write flags
	uchar startblock;		//Used to indicate read multiple blocks, indicating the starting block			
	uchar count;			//Used to describe how many blocks to read
	char status;			//ioctl status
...
typedef enum {
	...
	/**事件机制相关NDK返回值*/
	NDK_ERR_EVENT_UNREGISTER	   =	-3301, 	/**事件未注册*/
	NDK_EVENT_RFID_BUSY 		   = -3302,	/**非接卡事件忙，非接设备被占用*/
	NDK_ERR_EVENT_UNREALIZED	   = -3303,    /**事件未注册，该功能不支持*/
	...
	COM_FAIL= -6000,
	NDK_ERR_GM_SM2_3_BUSY = -6001,                      /**<国密SM2或SM3状态忙*/
}EM_NDK_ERR;
...
#define	 K_QUIT		0x9B	/**<退出输入*/
#define	 K_CLEAR	0x9C	/**<清空输入*/
...
typedef enum {
	...
	PRN_HZ_FONT_16x24 ,
	PRN_HZ_FONT_16x16BL,			/*16x16 粗体*/
	PRN_HZ_FONT_24x24BL,			/*24x24 粗体*/
	PRN_HZ_FONT_48x24A ,
	PRN_HZ_FONT_48x24B ,
	PRN_HZ_FONT_48x24C ,
	PRN_HZ_FONT_24x48A ,
...
typedef enum {
    ...
    PRN_ZM_FONT_12x32B,
    PRN_ZM_FONT_12x32C,
    PRN_ZM_FONT_8x16BL,					/*8x16 粗体,枚举值 36*/
    PRN_ZM_FONT_16x16BL,				/*不支持*/
    PRN_ZM_FONT_12x24BL					/*12x24 粗体,枚举值 38*/
}EM_PRN_ZM_FONT;
...
/** @addtogroup 文件系统 操作K21端文件新增的接口，相当于原来的文件系统接口
* @{
*/
/**
*@brief		打开文件.
...
*/
int NDK_SP_FsOpen(const char *pszName, const char *pszMode);
...
*/
int NDK_SP_FsClose(int nHandle);
...
*/
int NDK_SP_FsRead(int nHandle, char *psBuffer, uint unLength);
...
*/
int NDK_SP_FsWrite(int nHandle, const char *psBuffer, uint unLength);
...
*/
int NDK_SP_FsSeek(int nHandle, ulong ulDistance, uint unPosition);
...
*/
int NDK_SP_FsDel(const char *pszName);
...
*/
int NDK_SP_FsFileSize(const char *pszName, uint *punSize);
...
*/
int NDK_SP_FsRename(const char *pszSrcName, const char *pszDstName);
...
*/
int NDK_SP_FsExist(const char *pszName);
...
*/
int NDK_SP_FsTruncate(const char *pszPath, uint unLen);
...
*/
int NDK_SP_FsTell(int nHandle, ulong *pulRet);
...
typedef enum{
	...
	SEC_MAC_AES,	   /**< 数据分为16字节的block，不足补0，每个block进行AES加密后与下一个block异或后按AES加密*/
...
typedef enum {
	...
	SEC_PIN_ISO9564_4,     /**<16字节的PIN BLOCK FMT4*/
}EM_SEC_PIN;
...
/**
* nLenofOwnerBuff : 应用传入的Owner缓存长度
* pszOwner : 当前应用设置的KeyOwner值（应用端）
*/
int NDK_SecGetKeyOwner(int nLenOfOwnerBuffer,char *pszOwner);
...
/**
 *@brief    读取磁卡缓冲区的1、2、3磁道的二进制数据或者时间宽度数据
 *@retval   type  数据类型，二进制数据或时间宽度数据
 *@retval   track   读取的磁道
 *@retval   offset  偏移量
 *@retval   unLen   读取的长度
 *@retval   tkdata  存储数据的buffer
 *@retval   pnReadlen   返回实际读到的数据长度
 *@return
 *@li   NDK_OK              操作成功
 *@li   \ref NDK_ERR "NDK_ERR"              操作失败(磁卡设备未打开)
*/
...
/**
 *@brief	设置IC卡上电电压、遵循的规范
 *@param	pnMode	上电模式(参考\ref POWERUP_STANDARD "POWERUP_STANDARD")
			ICC_EMV_MODE		 		EMV2000, 自动选择9600、19200、38400速率上电, 不支持PPS
			ICC_BPS_576_MODE		 	EMV2000, baud : 57600, 不支持PPS
			ICC_BPS_384_MODE		 	EMV2000, baud : 38400, 不支持PPS
			ICC_BPS_192_MODE		 	EMV2000, baud : 19200, 不支持PPS
			ICC_SSC_MODE			 	Social Security Card, 自动选择9600、19200、38400速率上电, 不支持PPS
			ICC_ISO7816_PPS_MODE	 	ISO7816, baud : 9600, 支持PPS,
			ICC_ISO7816_BPS_96_MODE 	ISO7816, baud : 9600, 不支持PPS
			ICC_ISO7816_DEFAULT_MODE  	ISO7816, 自动选择9600、19200、38400速率上电, 不支持PPS
 *@param	pnVoltage	上电电压(参考\ref POWERUP_VOL "POWERUP_VOL")
			ICC_VOL_3V	  	3V
			ICC_VOL_5V  	5V
			ICC_VOL_1P8V	1.8V
 *@return
 *@li	NDK_OK				操作成功
 *@li	\ref NDK_ERR_PARA "NDK_ERR_PARA" 	参数非法(mode、voltage取值不在范围内)
 *@li	\ref NDK_ERR_IOCTL "NDK_ERR_IOCTL" 	驱动调用错误
*/
int NDK_IccSetPowerUpMode(int pnMode, int pnVoltage);

/**
 *@brief	获取卡所使用的传输协议类型（只支持IC卡和SAM卡）
 *@param	emIcType	卡类型(参考\ref EM_ICTYPE "EM_ICTYPE")
 *@retval	pnProtocol	卡片使用的传输协议类型
 *@return
 *@li	NDK_OK				操作成功
 *@li	\ref NDK_ERR_IOCTL "NDK_ERR_IOCTL" 	驱动调用错误
*/
...
/**
 *@brief  CPU间事件通知(例如K21通知安卓进行重启／安全触发事件等等)
*/
...
int  NDK_SysGetCapability(int nSizeOfCap, char* szCap);
...
typedef struct {
	char sEssid[WIFI_IW_ESSID_MAX_SIZE + 1];    /**<路由器名称*/
 ...
 *@return
 *@li	NDK_OK				操作成功
 *@li	\ref NDK_ERR_WIFI_DEVICE_FAULT "NDK_ERR_WIFI_DEVICE_FAULT"	WIFI-设备状态出错
 *@li	\ref NDK_ERR_WIFI_CMD_UNSUPPORTED "NDK_ERR_WIFI_CMD_UNSUPPORTED"	WIFI-不支持的命令
 *@li	\ref NDK_ERR_WIFI_SEARCH_FAULT "NDK_ERR_WIFI_SEARCH_FAULT"	WIFI-扫描状态出错
 ...
 *@li	\ref NDK_ERR_WIFI_DEVICE_TIMEOUT "NDK_ERR_WIFI_DEVICE_TIMEOUT"	WIFI-设备超时
 *@li	\ref NDK_ERR_WIFI_DEVICE_FAULT "NDK_ERR_WIFI_DEVICE_FAULT"	WIFI-设备状态出错
*/
 ...
 *@li	\ref NDK_ERR_WIFI_PROCESS_INBADSTATE "NDK_ERR_WIFI_PROCESS_INBADSTATE"	WIFI-无法连接到AP
*/
 ...
 *@param 无
 *@return
 *@li NDK_ERR_ICCARD_BUSY IC卡忙
 *@li NDK_ERR_RFID_BUSY   射频忙
 *@li NDK_ERR_PRN_BUSY    打印机忙
 *@li NDK_ERR_PIN_BUSY    触摸屏忙
 *@li NDK_ERR_IOCTL       IOCTL调用失败
操作成功
 *@li NDK_OK 系统处于空闲状态
操作失败
*/
...
/**
 *@brief	休眠接口，发送休眠指令给K21，等K21返回后再进入休眠
 *@return
 *@li	NDK_OK				操作成功
*/
...
*@return     返回值
*@li			NDK_OK(0)  成功
*@li			NDK_ERR_POSNDK_PERMISSION_UNDEFINED(-4021) 权限未声明
*@li			NDK_ERR_POSNDK_EVENT_NUM(-4006)  错误的事件号
*@li			NDK_ERR_PARA(-6) - 参数错
*@li			NDK_EVENT_BUSY(-4022)  该事件已经被占用了
*@li			NDK_ERR_POSNDK_EVENT_INIT(-4010) 初始化错
*/
int NDK_SysResumeEvent(EM_SYS_EVENT event);
/**
*@brief      射频卡卡片复位
*/
int NDK_RfidPiccResetCard(uchar usDelayMs);
/**
*@brief     事件挂起
*/
int NDK_SysSuspenedEvent(EM_SYS_EVENT eventNum);
//add by zengxiany for ISO15693
int  NDK_Iso15693_init(void);
int  NDK_Iso15693_Deinit(void);
int NDK_Iso15693_Inventory(iso15693NumSlots_t slotcnt, uchar maxCards,uint* Cardcount,iso15693ProximityCard_t* Mcards);
int NDK_Iso15693SelectPicc(iso15693ProximityCard_t Mcards);
int NDK_Iso15693GetPicc_SystemInfo(iso15693ProximityCard_t Mcards,iso15693PiccSystemInformation_t* sysInfo);
int NDK_Iso15693Read_SingleBlock(iso15693ProximityCard_t Mcards, uint ucBlockNum, iso15693PiccMemoryBlock_t* memBlock );
int NDK_Iso15693Read_MultipleBlocks(iso15693ProximityCard_t Mcards,uint startblock,uint icount,iso15693PiccMemoryBlock_t* memBlock);
int NDK_Iso15693Write_SingleBlock(iso15693ProximityCard_t Mcards,iso15693PiccMemoryBlock_t memBlock,uint ucBlockNum);
