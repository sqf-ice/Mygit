...
typedef enum{
	LCD_ROLLINGBACK = 1,	/**液晶正常显示*/
	LCD_ROLLINGOVER = 2		/**液晶翻转显示*/
}EM_LCD_ROLLING_STATUS;

/**
 *@brief	屏幕显示模式。
 *@details  只有正常显示和180度翻转显示
 *@param	mode	1 - 正常显示   	2 - 180度翻转显示
 *@return
 *@li	NDK_OK				操作成功
 *@li	\ref NDK_ERR_PARA "NDK_ERR_PARA" 		参数非法
*/
int NDK_ScrDispMode(EM_LCD_ROLLING_STATUS mode);

/**
 *@brief	设置按键音大小
 *@details  取值范围1~10,音量从小到大
 *@param	val,音量大小(1~10)
 *@return
 *@li	NDK_OK				操作成功
 *@li	NDK_ERR_OPEN_DEV	打开设备失败
 *@li	NDK_ERR_IOCTL		ioctl调用失败
 *@li	\ref NDK_ERR_PARA "NDK_ERR_PARA" 		参数非法
*/
int NDK_SysBeepVoiceCtrl(uint val);
...
/**
 *@brief	设置液晶背光亮度
 *@param	val 背光亮度,取值范围(1~20)
 *@return
 *@li	NDK_OK				操作成功
 *@li	\ref NDK_ERR_PARA "NDK_ERR_PARA" 		参数非法
*/
int NDK_ScrSetBrightness(int val);
...
/**
 *@brief	设置网络MAC地址
 *@retval	pszMacAddr	输入mac地址，以（FF-FF-FF-FF-FF-FF为类型字符串输入，并且是16进制的，比如00-80-BC-AD-00-00)
 *@return
 *@li	NDK_OK				操作成功
 *@li	\ref NDK_ERR_PARA "NDK_ERR_PARA" 		参数非法(pszMacAddr为NULL)
 *@li	\ref NDK_ERR "NDK_ERR" 				操作失败(获取mac地址失败)
*/
int NDK_EthSetMacAddr(const char *pszMacAddr);
/**
...
typedef enum {
    ...
    SYS_HWINFO_GET_WIFIFW_VER = 100,    /**<获取wifi固件版本号*/
} EM_SYS_HWINFO;
...
typedef enum{
	...
	SS_WLS_50N_GPRS = (SS_TYPE_WLS<<16|3),
 ...
 *@details  emFlag仅支持SYS_HWINFO_SET_BOARD_VER\SYS_HWINFO_SET_BOARD_NUMBER
...
int n_total; /*总执行次数*/
int n_suc; /*总成功次数*/
int n_fail_1; /*总fail_1次数*/
int n_fail_2; /*总fail_2次数*/
}gprs_erf_detail_t;
typedef struct{
gprs_erf_detail_t details[6];
}gprs_erf_info_t;
 ...
 *@retval   pValue 	统计值（该统计值是一个累加值，比如打印米数统计从第一打印米数一直累加所得值）
 *@return
 *@li	 NDK_OK				操作成功
 *@li	 \ref NDK_ERR "NDK_ERR"	操作失败(统计服务dbus通讯失败)
 *@li	 \ref NDK_ERR_PARA "NDK_ERR_PARA"	参数非法(pulValue为NULL、emDevId小于0)
*/
int NDK_SysGetStatisticsData(EM_SS_DEV_ID emDevId,void* pValue);
...
typedef enum{
        ...
        VOICE_CTRL_GET_VOLUME = 7, /**<音量获取*/
        VOICE_CTRL_BEPP_VOLUME = 8,  /**<蜂鸣器音量控制*/      
        VOICE_CTRL_FORCE_32_BIT= 0x7FFFFFFE /**Force enum to be stored in 32 bit variable*/
}EM_VOICE_CTRL;
/**
 *@brief	加载音频文件
...
*/
 ...
 int NDK_KbBeepControl(EM_SYS_KBBEEP control);
/** @} */ // 音频模块结束
...
/**
  *@brief flash读写 
  *@param addr:要写入或读取的flash的绝对地址，必须为4k的整数倍
  *@param buf:要写入或读取的数据缓存
  *@param len:要写入或读取的数据长度，写入时必须为4k的整数倍
  *@param opflag：写入或读取标志，0-读取数据，1-写入数据
  *@return
  *@li   NDK_OK                  操作成功返回
  *@li   \ref NDK_ERR_IOCTL      驱动调用错误
  *@li   \ref NDK_ERR                操作失败
 */
 int NDK_AppParamOp(uint addr, uchar *buf, uint len,uchar opflag);
 /**
  *@brief flash写函数
  *@param addr:要写入的flash绝对地址，必须为4k的整数倍
  *@param buf:要写入的数据缓存
  *@param len:要写入的数据长度，必须为4k的整数倍
  *@return
  *@li   NDK_OK                  操作成功返回
  *@li   \ref NDK_ERR_IOCTL      驱动调用错误
  *@li   \ref NDK_ERR                操作失败
 */
 int NDK_FlashWrite(uint addr, uchar *buf, uint len);
 /**
  *@brief flash擦除函数
  *@param addr:要擦除的flash绝对地址，必须为4k的整数倍
  *@param len:要擦除的数据长度，必须为4k的整数倍
  *@return
  *@li   NDK_OK                  操作成功返回
  *@li   \ref NDK_ERR_IOCTL      驱动调用错误
  *@li   \ref NDK_ERR                操作失败
 */
 int NDK_FlashErase(uint addr, uint len);
/**
 *@brief 单个数码管全亮或全灭
