...
/**
 *@brief		设置屏幕对比度。
 *@param		unContrast	对比度级别为0~7，0最淡，7最深。
 *@return
 *@li	NDK_OK				操作成功
 *@li	其它EM_NDK_ERR		操作失败
*/
...
/**
 *@brief	设置打印页长
 *@details   对打印机打印页长进行设置，该api只对针打生效
 *@param  len	页长
 *@return
 *@li	NDK_OK				操作成功
 *@li	其它EM_NDK_ERR		操作失败
 ...
 *@li	其它EM_NDK_ERR		操作失败
*/
int NDK_AppDoEvent(const uchar *pszFilename,const void *psInEventMsg, int nInlen, void *psOutEventMsg, int nMaxOutLen, int *pnOutLen);
...
typedef struct {
    ushort bits;       							/**< 模位数 */
    uchar modulus[MAX_RSA_MODULUS_LEN];      	/**< 模 */
	uchar publicExponent[MAX_RSA_MODULUS_LEN]; 	/**< 公钥指数 */
    uchar exponent[MAX_RSA_MODULUS_LEN];     	/**< 私钥指数 */
    uchar prime[2][MAX_RSA_PRIME_LEN];      	/**< pq素数 */
    uchar primeExponent[2][MAX_RSA_PRIME_LEN]; 	/**< 素数与指数除法值 */
    uchar coefficient[MAX_RSA_PRIME_LEN];  	 	/**< 素数与素数除法值 */
}ST_RSA_PRIVATE_KEY;
...
typedef struct {
    ushort bits;                    			/**< 模位数 */
    uchar modulus[MAX_RSA_MODULUS_LEN];  		/**< 模 */
    uchar exponent[MAX_RSA_MODULUS_LEN];		/**< 指数 */
}ST_RSA_PUBLIC_KEY;
 ...
 *@param	psModule		模缓冲
 *@param	nModuleLen		模长度
 *@param	psExp			存放RSA运算的指数缓冲区指针。就是e.按高位在前,低位在后的顺序存储
 *@param	nExpLen			指数字节长度
 *@param	psDataIn		数据缓冲
 *@retval	psDataOut		输出数据
 *@return
 *@li	NDK_OK				操作成功
 *@li	其它EM_NDK_ERR		操作失败
*/
int NDK_AlgRSARecover(uchar *psModule, int nModuleLen, uchar *psExp, int nExpLen, uchar *psDataIn, uchar *psDataOut);
 ...
 *@retval	psIccRespOut	卡片应答码,该参数传入NULL时，PIN结果通过NDK_SecGetPinResult函数获取
 *@return
 *@li	NDK_OK				操作成功
 *@li	其它EM_NDK_ERR		操作失败
*/
...
/**
 *@brief	设置网络地址（设置后如果原已动态分配的地址，也会被修改）
 *@param	pszIp	本地IP 地址字符串的首指针,不可为空指针.仅支持IPV4协议
 *@param	pszMask	本地子网掩码字符串的首指针,不可为空指针.仅支持IPV4协议
 *@param	pszGateway	本地网关地址字符串的首指针,不可为空指针.仅支持IPV4协议
 *@param 	pszDns	本地DNS服务器IP地址,为NULL则不设DNS地址
 *@return
 *@li	NDK_OK				操作成功
 *@li	其它EM_NDK_ERRCODE		操作失败
*/
 ...
 *@brief	获取网络地址
 *@retval	pszIp	返回IP地址
 *@retval	pszGateway	返回网关地址
 *@retval	pszMask	返回子网掩码
 *@retval	pszDns	DNS服务器IP地址,为空则不取DNS地址
 *@return
 *@li	NDK_OK				操作成功
 *@li	其它EM_NDK_ERR		操作失败
*/
