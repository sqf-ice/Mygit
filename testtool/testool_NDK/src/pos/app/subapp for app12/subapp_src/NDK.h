...
typedef enum {
	...
	DISP_TTF_INPUT_UTF8=3,					/**<显示TTF字体，输入格式UTF8*/
}EM_DISPTTFINPUT;
 ...
 *@details  系统默认支持3种(黑白2种)尺寸点阵大小的字体可供切换,不同机型设定的系统默认字体尺寸不一定相同\n\n
			彩屏支持:\n汉字16x16 ASCII:8x16 (DISPFONT_CUSTOM)\n
					 ...
					 汉字32x32 ASCII:16x32 (DISPFONT_EXTRA_LARGE)\n
...
*/
int NDK_SrcSetTrueTypeFontSizeInPixel(int width, int height);
 ...
 *@li   其它EM_NDK_ERR      操作失败
*/
int NDK_SrcGetTrueTypeFontSize(const char *str, int stringLength, int *width, int *height);
...
typedef enum {
	...
	PRN_TTF_INPUT_UTF8=3,					/**<打印TTF字体，输入格式UTF8*/
}EM_PRNTTFINPUT;
