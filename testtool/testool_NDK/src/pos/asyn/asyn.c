...
* DATE			: 20130114
* directory 		: 
* description		: asyn模块总入口,本模块的公共函数与公共定义也可以放于此处
...
/****************************************************************
* function name 	 	: asyn
* functional description 	: asyn模块入口主函数,可根据需要扩展
* input parameter	 	:
* output parameter	 	: 
* return value		 	:
...
*****************************************************************/
void asyn(PF* list, unsigned char id[])//入口函数名就是主文件名,也是模块名
{
	//以下为局部变量
	int ret=-1;
	
	//以下为函数体
	cls_show_msg1(5,"%s测试,请打开PC端后台并接连好线路,任意键继续...", TESTMODULE);
