...
void asyn10(void)
{
	...
	if((ret=NDK_MdmAdapt(MODEM_DIAL_TYPE_ASYN, NULL))!=NDK_ERR_PARA)
	{
		send_result("line %d:%s测试失败(ret=%d)", __LINE__, TESTAPI, ret);
		RETURN;
	}
	...
	if((ret=NDK_MdmAdapt(MODEM_DIAL_TYPE_ASYN, overload))!=NDK_ERR_PARA)
	{
		send_result("line %d:%s测试失败(ret=%d)", __LINE__, TESTAPI, ret);
		RETURN;
	}
	...
	if((ret=NDK_MdmAdapt(MODEM_DIAL_TYPE_ASYN, "errno123"))!=NDK_ERR_MODEM_SELFADAPTFAIL)
	{
		send_result("line %d:%s测试失败(ret=%d)", __LINE__, TESTAPI, ret);
		RETURN;
	}
	...
	if((ret=NDK_MdmAdapt(MODEM_DIAL_TYPE_ASYN, MDMDialStr))!=NDK_ERR_MODEM_OTHERMACHINE)
	{
		send_result("line %d:%s测试失败(ret=%d)", __LINE__, TESTAPI, ret);
		RETURN;
	}
	...
	if((ret=NDK_MdmAdapt(MODEM_DIAL_TYPE_ASYN, MDMDialStr))!=NDK_ERR_MODEM_NOLINE)
	{
		send_result("line %d:%s测试失败(ret=%d)", __LINE__, TESTAPI, ret);
		RETURN;
	}
	...
	if((ret=NDK_MdmAdapt(MODEM_DIAL_TYPE_ASYN, MDMDialStr))!=NDK_ERR_MODEM_SELFADAPTCANCEL)
	{
		send_result("line %d:%s测试失败(ret=%d)", __LINE__, TESTAPI, ret);
		RETURN;
	}
	...
	if((ret=NDK_MdmAdapt(MODEM_DIAL_TYPE_ASYN, MDMDialStr))!=NDK_OK)
	{
		send_result("line %d:%s测试失败(ret=%d)", __LINE__, TESTAPI, ret);
		RETURN;
	}
	sleep(3);//同步拨号的时候有去打补丁之类的，这样无形中增加了点时间，而异步拨号没做这个,故需要加延时(产品线反馈)20180103
	...
	rlen = sizeof(rbuf);//防止SDLC_read没有对rlen做清0
