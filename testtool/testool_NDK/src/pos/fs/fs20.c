...
void fs20(void)//无参数无返回
{
	...
	//创建非pad端创建源文件
	if((fp=NDK_FsOpen(SOURCEFILE1, "w"))<0)
	{
	...
	//判断两个文件内容是否一致
	if((ret=NDK_FsFileSize(DESTFILE, &size))!=NDK_OK)
	{
		send_result("line %d: %s测试失败(%d,%d)", __LINE__, TESTAPI, ret, size);
		RETURN;
	}
	if((fp=NDK_FsOpen(DESTFILE, "r"))<0)
	{
		send_result("line %d:%s测试失败(%d)", __LINE__, TESTAPI, fp);
		RETURN;
	}
	if((readlen=NDK_FsRead(fp, readbuf, size))!=size)
	{
	...
	if((fp=NDK_FsOpen(DESTFILE3, "w"))<0)
	{
		send_result("line %d:%s测试失败(%d)", __LINE__, TESTAPI, fp);
		RETURN;
	}
	for (i=0; i<sizeof(writebuf); i++)
		writebuf[i]=rand()%256;
	if((writelen=NDK_FsWrite(fp, writebuf, 1024))!=1024)
	{
	...
	if((ret=NDK_FsFileSize(DESTFILE3, &size1))!=NDK_OK)
	{
	...
	if((readlen=NDK_FsRead(fp, readbuf, size))!=sizeof(str2))
	{
		send_result("line %d:%s测试失败(%d)", __LINE__, TESTAPI, readlen);
		NDK_FsClose(fp);
		NDK_FsDel(DESTFILE);
