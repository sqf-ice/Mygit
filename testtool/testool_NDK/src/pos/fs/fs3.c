...
void fs3(void)
{
	...
	if((fp=NDK_FsOpen(FILETEST3, "w"))<0) // 创建文件失败
	{
		send_result("line %d: %s测试失败(%d)", __LINE__, TESTAPI, fp);
		GOTOERR;
	}	
	// 对空文件进行移指针(SEEK_CUR [0/1/-2])
	if((ret=NDK_FsSeek(fp, 0l, SEEK_CUR))!=NDK_OK) // 空文件移指针SEEK_CUR位置0,应该成功
	{
		send_result("line %d: %s测试失败(%d)", __LINE__, TESTAPI, ret);
		GOTOERR;
	}
	if((ret=NDK_FsSeek(fp, 1l, SEEK_CUR))!=NDK_OK) // 空文件移指针SEEK_CUR位置1,应返回成功
	{
		send_result("line %d: %s测试失败(%d)", __LINE__, TESTAPI, ret);
		GOTOERR;
	}
	//此时位于EOF之后的下一个字节位置,-2l以构造超出文件头的情况
	if((ret=NDK_FsSeek(fp, -2l, SEEK_CUR))!=NDK_ERR) // 空文件移指针SEEK_CUR位置-2,应返回失败
	{
		send_result("line %d: %s测试失败(%d)", __LINE__, TESTAPI, ret);
		GOTOERR;
	}
	...
	if((writelen=NDK_FsWrite(fp, writebuf, 200))!=200) // 写入200字节数据
	{
...
#if !defined ME15C	//不支持NDK_FsTell函数屏蔽指针位置验证函数
	else
	{
	#if (K21_ENABLE&&!ANDIROD_ENABLE)
		if((ret=NDK_FsTell(fp, &pulRet))!=NDK_OK||pulRet!=200) //只会到200
	#else
...
#if !defined ME15C	//不支持NDK_FsTell函数屏蔽指针位置验证函数
		if((ret=NDK_FsTell(fp, &pulRet))!=NDK_OK||pulRet!=seeklen) // 偏移后指针位置不对
		{
			send_result("line %d: %s测试失败(%d)", __LINE__, TESTAPI, ret);
			GOTOERR;
		}
	...
	{
		if((ret=NDK_FsTell(fp, &pulRet))!=NDK_OK||pulRet!=200) // 指针位置错
		{
			send_result("line %d: %s测试失败(%d)", __LINE__, TESTAPI, ret);
			GOTOERR;
		}
	...
	{
#if (K21_ENABLE&&!ANDIROD_ENABLE)  //只会到200
	...
	if((readlen=NDK_FsRead(fp, readbuf, 200))!=100) // 读取校验内容
	{
