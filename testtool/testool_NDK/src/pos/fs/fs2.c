...
#define  FILETEST2 "FT2"
#else //K21平台只支持绝对路径
#define  FILETEST2 "/appfs/FT2"
#endif
#define 	TESTAPI		"NDK_FsClose"
...
void fs2(void)
{
	...
	//测试前置
	NDK_FsDel(FILETEST2); // 确保打开的是一个空的测试文件

	//case1:创建一个空文件，删除一个刚创建的文件，应该成功返回。
	if((fp=NDK_FsOpen(FILETEST2, "w"))<0)
	{
	...
	if((fp=NDK_FsOpen(FILETEST2, "r"))<0)
	{
		send_result("line %d: %s测试失败(%d)", __LINE__, TESTAPI, fp);
		GOTOERR;
	}
	...
	if((ret=NDK_FsClose(113))!=NDK_ERR)
	{
		send_result("line %d: %s测试失败(%d)", __LINE__, TESTAPI, ret);
		GOTOERR;
	}
	...
	//测试清理
	if((ret=NDK_FsDel(FILETEST2))!=NDK_OK)
	{
		send_result("line %d: %s测试失败(%d)", __LINE__, TESTAPI, ret);
		GOTOERR;
	}

	if((ret=NDK_FsClose(MIN_SIGNINT))!=NDK_ERR)
	{
		send_result("line %d: %s测试失败(%d)", __LINE__, TESTAPI, ret);
		GOTOERR;
	}	
	if((ret=NDK_FsClose(MAX_SIGNINT))!=NDK_ERR)
	{
		send_result("line %d: %s测试失败(%d)", __LINE__, TESTAPI, ret);
		GOTOERR;
	}
