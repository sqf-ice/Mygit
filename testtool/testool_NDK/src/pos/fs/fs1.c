...
#define  FILETEST1	"fsys1"			//相对路径
#define  APPFSFILE	"/tmp/appfsfsys"	//绝对路径
#define	 NONLICET	"/"
...
* functional description : 文件系统模块的1号用例主函数
* input parameter	  	 :
* output parameter	 	 : 
* return value		 	 :
...
void fs1(void)
{
	...
	if ((fp=NDK_FsOpen(FILETEST1, "w"))<0) // 关闭文件后以写的文件创建该文件
	{
		send_result("line %d: %s测试失败(%d)", __LINE__, TESTAPI, fp);
		GOTOERR;
	}
	NDK_FsClose(fp);
	NDK_FsDel(FILETEST1);
	if ((fp=NDK_FsOpen(FILETEST1, "w"))<0) // 以写的文件打开刚刚删除的文件
	{
		send_result("line %d: %s测试失败(%d)", __LINE__, TESTAPI, fp);
		GOTOERR;
	}
	memset(writebuf, 0x55, 200);
	if((writelen=NDK_FsWrite(fp,writebuf, 200))!=200) // 写入一定数据(0x55)
	{
		send_result("line %d: %s测试失败(%d)", __LINE__, TESTAPI, writelen);
		GOTOERR;
	}
	NDK_FsClose(fp);
	if ((fp=NDK_FsOpen(FILETEST1, "w"))<0) // 以写的文件打开刚刚创建的文件
	{
		send_result("line %d: %s测试失败(%d)", __LINE__, TESTAPI, fp);
		GOTOERR;
	}
	memset(writebuf, 0x88, 100);
	if((writelen=NDK_FsWrite(fp,writebuf, 100))!=100) // 覆盖写入数据(0x88)
	{
		send_result("line %d: %s测试失败(%d)", __LINE__, TESTAPI, writelen);
		GOTOERR;
	}
	NDK_FsClose(fp);
	if ((fp=NDK_FsOpen(FILETEST1, "r"))<0) // 检查是否能打开程序运行过程中创建的文件
	{
		send_result("line %d: %s测试失败(%d)", __LINE__, TESTAPI, fp);
		GOTOERR;
	}
	...
	if((readlen=NDK_FsRead(fp,readbuf, 200))!=200)//此处其实也验证了fopen("w")并不是新建文件
	{
...
#if !defined SP610	//郑凯解释SP610底层会增加上"appfs/"而低端平台"appfs//"是合法文件名,故SP610不测试该用例
	if ((fp=NDK_FsOpen(NONLICET, "w"))!=NDK_ERR_OPEN_DEV)
	{	
		send_result("line %d: %s测试失败(%d)", __LINE__, TESTAPI, fp);
		NDK_FsClose(fp); //目前发现打开"/"文件会成功，需关闭该句柄，否则后面测试会受影响 20160729 linwl
		GOTOERR;
	}
#endif	
	...
	memset(writebuf, 0x88, 100);
	if((writelen=NDK_FsWrite(fp2,writebuf, 100))!=100) // 覆盖写入数据(0x88)
	{
		send_result("line %d: %s测试失败(%d)", __LINE__, TESTAPI, writelen);
		GOTOERR2;
	}
	NDK_FsClose(fp2);
	if ((fp2=NDK_FsOpen(APPFSFILE, "r"))<0) // 检查是否能打开程序运行过程中创建的文件
	{
		send_result("line %d: %s测试失败(%d)", __LINE__, TESTAPI, fp2);
		GOTOERR2;
	}
	...
	memset(readbuf, 0, sizeof(readbuf));
	if((readlen=NDK_FsRead(fp2,readbuf, 100))!=100)//此处其实也验证了fopen("w")并不是新建文件
	{
		send_result("line %d: %s测试失败(%d)", __LINE__, TESTAPI, readlen);
		GOTOERR2;	
	}
	...
	if ((fp=NDK_FsOpen(FILETEST1, "w"))<0)//构造测试用文件
	{
		send_result("line %d: %s测试失败(%d)", __LINE__, TESTAPI, fp);
		GOTOERR;
	}
	if((writelen=NDK_FsWrite(fp,"0123456789", 10))!=10) // 写入一定数据"0123456789"
	{
		send_result("line %d: %s测试失败(%d)", __LINE__, TESTAPI, writelen);
		GOTOERR;	
	}
	...
	if ((fp=NDK_FsOpen(FILETEST1, "r"))<0)//第一次打开文件,句柄fp
	{
		send_result("line %d: %s测试失败(%d)", __LINE__, TESTAPI, fp);
		GOTOERR;
	}
	memset(readbuf,0,sizeof(readbuf));
	if((readlen=NDK_FsRead(fp,readbuf, 5))!=5)//读取5个字节数据
	{
		send_result("line %d: %s测试失败(%d)", __LINE__, TESTAPI, readlen);
		GOTOERR;		
	}
	...
	if ((fp2=NDK_FsOpen(FILETEST1, "r"))<0)//第二次打开文件,句柄fp2
	{
		send_result("line %d: %s测试失败(%d)", __LINE__, TESTAPI, fp);
		GOTOERR;
	}
	memset(readbuf,0,sizeof(readbuf));
	if((readlen=NDK_FsRead(fp2,readbuf, 10))!=10)//读取10个字节数据
	{
		send_result("line %d: %s测试失败(%d)", __LINE__, TESTAPI, readlen);
		GOTOERR;		
	}
