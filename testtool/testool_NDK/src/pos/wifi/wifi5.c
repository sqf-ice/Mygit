...
void verify_relink(void)
{
	...
	//case5:设置为静态IP，应该能够成功自动重连(路由器的网段需为1，否则会报错)
	memset(&st_ap,0,sizeof(st_ap));
	strcpy(list_s.stored_ap->ssid,test_ap); 
	strcpy(list_s.stored_ap->pwd,pwd);
	strcpy(list_s.stored_ap->ip.ip,"192.168.1.10");
	strcpy(list_s.stored_ap->ip.netmask,"255.255.255.0");
	strcpy(list_s.stored_ap->ip.gateway,"192.168.1.1");
	strcpy(list_s.stored_ap->ip.dns_primary,"218.85.157.99");
	strcpy(list_s.stored_ap->ip.dns_secondary,"192.168.30.1");
	list_s.stored_ap->priority = 0;
	list_s.stored_num = 1;
	if((ret = NDK_WifiProfileSet(&list_s)) != NDK_OK)
	{
